...
class APCRefueling @JvmOverloads constructor(
	context: Context,
	...
	private fun calculatePercentageOfData() {
		totalAmount = dataList[0].first
		var percentFull = 100.0F;
		var startAt = circleSectionSpace
		percentageCircleList = dataList.mapIndexed { index, pair ->
			var percent = pair.first.toFloat() / 100.0f
			percent = if (percent >= 100f) 75F else percent
			percent = if (percent < 10f && percent > 1f) 25f else percent
			percent = if (percent < 1f && percent > 0.01f) 20f else percent
			percent = if (percent < 0.01f) 0f else percent
			percent = if (percent != 0f) percent - circleSectionSpace else 0f
			percent = if (index == 0) percent - circleSectionSpace else percentFull - circleSectionSpace
			percentFull -= percent
