...
public abstract class LoadDataView {
	...
	public static void loadViewIncome(Context context, TableLayout tableLayout) {
		...
		for (int i = 0; i < MainActivity.getBalances().size(); i++) {
			int k = 1;
			ItemTable item = MainActivity.getBalances().get(i);
			if (item.getGroup() == ItemTable.GROUP.INCOME) {
				...
				tableIncomeTextName.get(k).setText(item.getName());
				...
				tableIncomeTextMoney.get(k).setText(String.valueOf(item.getMoney()));
	...
	public static void loadViewExpense(Context context, TableLayout tableLayout) {
		...
		tableExpenseRows.get(0).addView(tableExpenseTextAmount.get(0), new TableRow.LayoutParams(
			TableRow.LayoutParams.FILL_PARENT, TableRow.LayoutParams.WRAP_CONTENT, 0.75f));
		...
		for (int i = 0; i < MainActivity.getBalances().size(); i++) {
			...
			if ((MainActivity.getBalances().get(i).getGroup() == ItemTable.GROUP.EXPENSE
				...
				tableExpenseTextProduct.get(k).setText(item.getName());
				...
				tableExpenseTextAmount.get(k).setText(String.valueOf(item.getAmount()));
				...
				tableExpenseTextMoney.get(k).setText(String.valueOf(item.getMoney()));
